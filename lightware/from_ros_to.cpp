...
LaserScan rosScanData;
lwScanRev scanRev;
int32_t userAngleOffset;

void initScanRev(lwScanRev* Scan, LaserScan& ScanMsg, std::string FrameId) {
    Scan->revId = 0;
...
void generateScanPacket(lwScanRev* Scan, LaserScan& ScanMsg) {
...
int driverStart(lwSerialPort** Serial, const char* PortName, int32_t BaudRate) {
    ...
    *Serial = serial;
	if (!serial->connect(PortName, BaudRate)) {
        // qDebug() << "Could not establish Serial connection\n";
        return 1;
	};
...
int main(int argc, char** argv) {
    ...
    ros::Publisher laserScanPub = n.advertise<LaserScan>("laserscan", 10);
    ...
    if (driverStart(&serial, portName.c_str(), baudRate) != 0) {
        // qDebug() << ("SF40C failed to start driver");
        return 1;
    }
    ...
    if (driverScanStart(serial) != 0) {
        // qDebug() << ("SF40C failed to start scan");
        return 1;
    }
